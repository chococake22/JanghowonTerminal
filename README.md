# JanghowonTerminal

## 장호원시외버스터미널 시간표 만들기

### 개요
- 장호원의 공식 주소는 경기도 이천시 장호원읍으로 읍 단위의 작은 지역이다.
- 장호원은 주소상 경기도에 속하지만 충북과 맞닿을 정도로 외곽에 위치하고 있으며 다른 수도권에 비해서 교통 인프라가 매우 부족하다.
- 특히 터미널의 경우 제대로 된 시설이 갖추어지지 않은 상태로 컨테이너 형식의 시설을 유지하고 있는 상태이다.
- 다음 카페에 터미널 시간표가 있기는 하지만 별도의 웹이나 앱 서비스는 존재하지 않는다.
- 이번 프로젝트는 장호원 터미널을 이용하는 사람들이 별도의 웹을 통해 사용할 수 있는 터미널 시간표 서비스를 구축하는 것을 목표로 한다.

### 요구사항

#### 회원

- 아이디, 비밀번호, 이메일, 전화번호가 있다.
- 회원가입을 하고 로그인을 할 수 있다.
- 이메일, 전화번호를 변경하고, 별도로 비밀번호를 변경할 수 있다.
- 공지사항에 글을 작성하고 수정, 삭제가 가능하다.
- 내 시간표 기능을 사용하여 원하는 버스 시간표를 추가할 수 있다.
- 댓글을 작성할 수 있다.

#### 게시판(공지사항)

- 제목, 작성자, 내용, 작성일자, 수정일자가 있다.
- 로그인한 경우에만 공지사항을 작성할 수 있다.
- 해당 작성자만 글 수정, 삭제가 가능하다.

#### 댓글

- 작성자, 내용이 있다.
- 회원만 댓글 작성이 가능하다.

#### 버스시간표

- 목적지, 출발시간, 요금, 경유지, 특이사항이 있다.
- 관리자만 버스 시간표를 새로 등록할 수 있다.
- 각 회원별 버스시간표를 추가할 수 있다.


### 개발환경
- Java 11 
- Gradle
- Spring Boot
- JPA(hibernate)
- Spring Security
- Mysql
- Thymeleaf
- IntelliJ

### 테이블 설계

<img width="1055" alt="스크린샷 2022-03-23 오전 11 10 20" src="https://user-images.githubusercontent.com/93370148/159609735-0752cb65-8d19-4b62-a489-2dfd5b9e1a68.png">

- Account : 회원
- AccountBusInfo : 회원별 버스 시간표를 등록하는 객체
- BusInfo : 버스 시간표
- Board : 게시판의 게시글
- Comment : 댓글

<설명>

- Comment와 Board는 다대일 양방향 관계다.
- Board 기준으로는 일대다 단방향 관계이지만 외래키가 Comment에 있는 경우 update 쿼리가 추가로 나가기 때문에 다대일 양방향을 통해 보완했다.

- Board와 Account는 다대일 양방향 관계다.
- 게시글을 볼 경우 Account에 대한 정보까지 같이 가져오게 된다.
- 일반적으로는 지연로딩을 권장하지만 현재 Board를 조회할 경우 Account의 username을 보여주기 위해서 즉시로딩(EAGER)로 설정했다.
- 이 부분에 대해서는 추후에 페치 조인과 같은 방법으로 보완할 예정이다.

- Account와 BusInfo는 다대다 양방향 관계인데 중간 객체(AccountBusInfo)를 통해 일대다, 다대일 관계로 풀어냈다.
- 회원은 자신이 주로 타는 목적지의 시간표를 내 시간표로 정할 수 있다.
- 또한 버스 시간표 입장에서는 누가 이 시간표를 내 시간표로 저장했는지를 파악한다.
- (당장의 비즈니스 상에서 큰 의미는 없을 수 있지만 나중에 이 날짜의 이 시간표에 얼마나 많은 사람이 탔는지 파악할 수 있을 거라고 판단했다.)
- 외래키는 전부 다쪽인 AccountBusInfo가 가지고 있는 일대다, 다대일 관계이다.


### 과정

2022-03-05 
 - 공지사항 게시판을 먼저 구현하였다.
 - 페이징 기능의 경우 정확한 쿼리를 보내야하는데 그에 맞는 th:with를 통한 변수 생성과 로직을 구현하는 것이 쉽지 않았다.
 - Dto 객체 Entity를 자유자재로 변환할 수 있는 형태로 만드는 것이 중요하다는 것을 알게 되었다.
 - 실제 화면에서 보여지는 것은 Dto를 통해서 전달받기 때문이다.

2022-03-06
 - 공지사항 게시판 및 시간표 조회 기능을 구현했다.
 - 게시판의 경우 모든 사람들이 게시글을 조회할 수 있도록 구현했다.
 - 하지만 게시글을 생성하고 수정, 삭제하는 기능은 관리자만 할 수 있도록 기능을 주는 것이 중요하다.

2022-03-07
 - 목적지에 맞는 버스 시간표를 불러올 수 있도록 기능을 구성했다.
 - 공지사항과 동일하게 페이징 기능을 구현했는데 404 오류가 발생했다.
 - 원인을 살펴보니 쿼리가 올바르지 못했고 타임리프에서 쿼리를 수정하여 제대로 작동하였다.
 - 해당 목적지에 배차가 몇 개나 있는지 총 건수를 추가하였다.

2022-03-08
 - Spring Security를 적용해서 페이지 별 접근을 구분했다.

2022-03-09
 - Spring Security가 적용된 회원 관리 기능을 추가했다.
 - 계정의 권한을 주는 부분은 확실히 공부가 더 필요하다.
 - 권한별로 보여지는 기능을 구분하여 관리자만 버스 시간표를 추가하고 공지사항을 작성할 수 있도록 했다.

2022-03-10
 - Thymeleaf에서 th:와 아닌 것의 차이를 서버 사이드 렌더링을 통해서 알게 되었다.
 - authentication 객체로부터 회원의 정보를 받아올 수 있도록 구현하였다.
 - Rest Api 구현을 시작하였다.
 - Spring Security에서 권한 설정을 통해 모든 회원이 공지사항에 게시글을 적을 수 있도록 수정했다.
 - JPA에서의 테이블 매핑에 대한 공부가 부족하다고 느꼈다.

2022-03-11
 - Account, Board, BusInfo 부분에 대한 Rest Api를 구현하였다.
 - 완벽히 한거라고 보기는 어려우나 Json 방식을 통해서 값을 받아오는 것을 Postman으로 확인하였다.
 - 내가 구현한 코드에 대해서 어떤 원리를 가지고 동작하는지 설명할 수 있을 정도로 공부할 필요성을 느꼈다.

2022-03-12
 - 게시글과 댓글의 연관관계에 대해서 공부하였다.
 - 댓글과 게시글이 다대일 단방향으로 댓글을 기준으로 게시글을 참조하게 되어있다.
 - 연관관계의 주인은 다쪽인 댓글이 된다.
 
2022-03-17
 - JUnit을 이용한 테스트 코드를 구현하기 시작했다.
 - 테스트 케이스 작성시 다양한 예외 상황을 생각할 수 있어야 의미있는 테스트가 된다는 것을 알게 되었다.

2022-03-18
 - 회원정보 수정시 비밀번호까지 동시에 수정이 되는 오류를 해결하였다.
 - 구글링을 통해서 변경 감지와 연관된 오류라는 것을 파악하고 원리에 맞게 로직을 수정하였다.

2022-03-19
 - 회원가입시 유효성 검사를 위해서 BindingResult를 사용하였다.
 - 아이디 부분에서 중복 체크 로직부분에서 오류가 있던 것을 service에서 repository로 바꾸면서 수정할 수 있었다.
 - 추후에 validator를 이용한 코드로 수정할 예정

2022-03-20
 - bindingResult를 이용해서 비밀번호 변경시 조건을 부여하였다.
 - 회원 별로 내 시간표를 구성할 수 있도록 기능을 추가하였다.
 - 테이블 매핑 간의 다대다 매핑을 위해서 AccountBusInfo 도메인을 추가하였다.
 - 매핑을 할 경우 단순히 논리적인 관계가 아닌 엔티티를 기준으로 한 객체 지향의 원리를 지키는 것이 중요함을 깨달았다.
 - 구현은 했으나 코드가 지저분해지는 부분이 많이 있는 것 같아서 보완이 많이 필요하다.

2022-03-22
 - Account와 Board를 일대다 연관관계로 매핑했다.(회원이 쓴 글이 무엇이 있는지를 확인할 수 있도록)
 - 이 과정에서 Board를 가져올 때 Account를 지연로딩으로 설정해서 serialization 오류가 났다.
 - 구글링을 통해서 해결 사항을 알게 되었고, 일단 Board만 Account를 같이 조회한다는 가정 하에 EAGER로 바꿨다.(추후에 다른 객체가 Account를 참조하게 된다면 이 부분도 수정해야 한다)
 - 또한 Account - AccountBusInfo - BusInfo의 일대다-다대일 연관관계 매핑에서 serialization(Infinite Recursion) 오류가 발생하였다.
 - 이유는 양쪽이 서로를 무한 참조하여 스택에 데이터가 넘쳐서 힙 영역을 침범했기 때문이다.
 - 이를 통해 직렬화에 대해서 공부하여 정리하였다.
 - 또한 @JsonBackReference 어노테이션을 통해 문제를 해결할 수 있었다.

2022-03-25
 - @AllargsConstructor를 통해서 필드 주입에서 생성자 주입으로 수정했다.
 - 필드 주입과 수정자 주입보다 생성자 주입을 권장하는 이유에 대해서 공부할 수 있었다.

2022-03-27
 - 테스트 코드를 작성하면서 비즈니스 로직처럼 Dto를 통해 접근해야하는가에 대한 고민을 했다.
 - 또한 다양한 예외상황에 대해서 테스트 코드를 작성해야 의미있는 테스트 코드가 된다는 것을 알게 되었다.
 - 코드가 내가 구현한 대로만 작동한다는 보장이 없고, 실제로 null 값이 있거나 JPA의 다양한 설정으로 인해서 엔티티가 훼손이 될 가능성이 있기 때문이다.
 - 잘은 모르지만 요즘은 TDD(테스트 주도 개발)로 개발을 하는 곳이 많고, 실제로 채용 공고에서 TDD에 대한 언급도 많았던 만큼 테스트 코드에 대한 공부가 필요하다.

2022-03-29
 - 테스트 코드 작성시 Mock을 사용하여 단위 테스트를 시도했다.
 - 실제로 Mock에 대한 이해가 많이 부족했기 때문에 다양하게 시도하면서 이해하는 것에 집중했다.
 - Mock에 대한 설명과 예제는 생각보다 많이 없는 것 같고, 그나마 있던 영어로 된 소스도 참고했다.

2022-03-30
 - @InjectMocks와 @Mock를 사용하는 방법을 공부했으나 실제로 테스트에는 성공하지 못했다.
 - Mock 객체가 외부 의존성(데이터베이스 연동, api 등)을 배제하고 로직의 상황에 집중해야할 경우 사용해야함을 알게 되었다.
 - 또한 테스트 데이터가 실제 DB에 연동이 되지 않고 단독적으로 사용하기 위해서 Mock을 사용한다는 것을 알 수 있었다.
 - 여전히 단위테스트를 하는 방법과 원리에 대해서는 이해가 부족하지만 얻는 것이 아무것도 없는 과정은 아니었다.
 - 기나긴 삽질 끝에 okky의 도움을 받아 @InjectMocks와 @Mock의 원리를 이해할 수 있었다.
 
 
2022-04-03
 - Controller, Service 등 layer 별로 테스트 설정을 다르게 해서 테스트해야함을 알게 되었다.
 - 실제로 given, assert 등의 메소드 활용에 대해서는 추가로 공부가 필요함을 느꼈다.
 - 애매하게 구글링으로 찾아보면서 하는 것보다는 클론 코딩을 통해서 전체적인 과정과 쓰임을 아는 것이 필요하다.

2022-04-12
 - swagger 적용으로 api를 더 효율적으로 개발하고 관리할 수 있게 하였다.
 - swagger-ui.html 설정에서 계속 404 오류로 인해서 시간이 좀 걸렸지만 리소스를 불러오는 것을 추가하여 문제를 해결하였다. 
